...
static int modeset_find_crtc(int fd, drmModeRes *res, drmModeConnector *conn,
                            struct modeset_dev *dev, struct modeset_output* out);
			     ...
			     struct modeset_dev* dev, struct modeset_output *out)
{
	...
	if (enc) {
		if (enc->crtc_id) {
			crtc = enc->crtc_id;
			if( atomic )
			for (itera = output_list; itera; itera = itera->next) {
				if (itera->crtc.id == crtc) {
					crtc = 0;
					break;
				}
			}
                             ...
                             struct modeset_dev *dev)
{
        ...
        ret = modeset_find_crtc(fd, res, conn, dev, NULL ); // assert atomic == false
        if (ret) {
...
struct modeset_dev_a { // must rename this structure
	struct modeset_dev_a *next;
...
static struct modeset_dev_a *modeset_list_a = NULL; // must rename this variable too.

static struct modeset_output *modeset_output_create(int fd, drmModeRes *res,
						    drmModeConnector *conn,
			     struct modeset_dev_a *dev)
{
	int ret;
	struct modeset_output *outa;
	//struct modeset_dev *out;

	/* creates an output structure */
	outa = malloc(sizeof(*outa));
	memset(outa, 0, sizeof(*outa));
	outa->connector.id = conn->connector_id;
	//out = malloc(sizeof(*out));
	//memset(out, 0, sizeof(*out));
	//out->connector = conn->connector_id;
	...
	/* copy the mode information into our output structure */
        //if( atomic )
	memcpy(&outa->mode, &conn->modes[0], sizeof(outa->mode));
	//else
...
//    if( atomic )
//    {
	if (drmModeCreatePropertyBlob(fd, &outa->mode, sizeof(outa->mode),
	                              &outa->mode_blob_id) != 0) {
		fprintf(stderr, "couldn't create a blob property\n");
		free( outa ); return NULL;
//		goto out_error;
	}
	fprintf(stderr, "mode for connector %u is %ux%u\n",
	        conn->connector_id, outa->bufs[0].width, outa->bufs[0].height);
//    }
/* else
    {
	...
	/* find a crtc for this connector */
	ret = modeset_find_crtc(fd, res, conn, NULL, outa);
	if (ret) {
		fprintf(stderr, "no valid crtc for connector %u\n",
			conn->connector_id);
		goto out_blob;
	}
	dev -> crtc = outa -> crtc.id;

	/* with a connector and crtc, find a primary plane */
	ret = modeset_find_plane(fd, outa);
	if (ret) {
	...
	ret = modeset_setup_objects(fd, outa);
	if (ret) {
	...
	ret = modeset_setup_framebuffers(fd, conn, outa);
	if (ret) {
...
out_obj:
	modeset_destroy_objects(fd, outa);
out_blob:
	drmModeDestroyPropertyBlob(fd, outa->mode_blob_id);
out_error:
	free(outa);
	return NULL;
}
...
static int modeset_prepare(int fd)
{
	...
	for (i = 0; i < res->count_connectors; ++i) {
		...
		if( atomic )
		{
		struct modeset_dev_a* dev = malloc( sizeof( struct modeset_dev_a ) );
		memset(dev, 0, sizeof(*dev));
		dev->conn = conn->connector_id;
		out = modeset_output_create(fd, res, conn, dev);
		if( out != NULL )
		{
		dev->saved_crtc = drmModeGetCrtc(fd, dev->crtc);
		dev -> next = modeset_list_a;
		modeset_list_a = dev;
		}
		else
		{
...
static void modeset_page_flip_event_a(int fd, unsigned int frame,
				    unsigned int sec, unsigned int usec,
...
static void modeset_draw(int fd)
{
	 ...
	 */
	ev.version = atomic ? 3 : 2;
	ev.page_flip_handler = modeset_page_flip_event;
	ev.page_flip_handler2 = modeset_page_flip_event_a;

	/* perform modeset using atomic commit */
	if( atomic ) modeset_perform_modeset(fd);
        else /* redraw all outputs */
        for (iter = modeset_list; iter; iter = iter->next) {
                iter->r = rand() % 0xff;
                iter->g = rand() % 0xff;
                iter->b = rand() % 0xff;
...
static void modeset_cleanup(int fd)
{
	struct modeset_output *itera = NULL;
	...
	if( atomic ) while( modeset_list_a != NULL )
	{
		struct modeset_dev_a* old = modeset_list_a;
/*
		printf( "%d %d %d %d\n",
			       old->saved_crtc->crtc_id,
			       old->saved_crtc->buffer_id,
			       old->saved_crtc->x,
			       old->saved_crtc->y
			       );
*/
	...
	if(atomic)
		...
		output_list = itera->next;


		/* destroy current output */
		modeset_output_destroy(fd, itera);
...
void usr2( int signal )
{
   printf( "0002\r\n" );
   fflush( stdout );
   ioctl( console, VT_RELDISP, VT_ACKACQ );
}
...
{
   int err = 0;
   err = ioctl( console, VT_SETMODE, &vtm );
   assert( err >= 0 );
   return EXIT_SUCCESS;
}
...
int vt_init()
{
   int err = 0;
   struct vt_mode vti;
   bzero( &vtm, sizeof( struct vt_mode ) );
   signal( SIGINT, term );
   signal( SIGTERM, term );
   signal( SIGSEGV, term );
   signal( SIGBUS, term );
   ...
   vti.relsig = SIGUSR1;
   vti.acqsig = SIGUSR2;
   err = ioctl( console, VT_SETMODE, &vti );
   assert( err >= 0 );
